...
過去の開発セッションで抽出された知見について、ドキュメント昇格の要否をトリアージする:

- 各 learnings アーティファクトの知見を1つずつ以下の基準で判定する
- 既に反映済みの知見は除外
- learnings アーティファクトが1つも存在しない場合は「対象なし」とする

**昇格すべき知見（adopt）**:
- このプロジェクト固有の規約やパターンで、コードベースを読むだけでは判断が難しいもの
- 繰り返し発生しうる問題で、ドキュメントに書くことで再発を防げるもの
- 既存のルールに追加条件や例外があり、明文化しないと誤った判断をしうるもの

**昇格すべきでない知見（skip）**:
- ライブラリのドキュメントを読めばわかること（API の型、メソッドの挙動など）
- テストやコーディングの一般的なベストプラクティス（「異常系もテストすべき」など）
- 特定のバージョンに依存する挙動で、アップデートにより陳腐化するリスクが高いもの
- 「当たり前のことだが今回たまたまレビューで指摘された」類の知見
...
- **判定**: adopt / skip
- **判定理由**: （1-2文で、なぜ昇格すべき/すべきでないか）
- **反映先**: docs/coding-rules/rules.md 等（adopt の場合のみ）
- **追記テキスト案**:（adopt の場合のみ。実際にドキュメントに追加する文面）
```
...
### Step 6: 処理済み learnings の削除

承認された提案の適用が完了した後、トリアージ済み（adopt/skip 問わず）の learnings.md を `learnings.reviewed.md` にリネームする。
learnings はフロー型情報からストック型情報（docs/）への変換バッファであり、トリアージが完了した時点で役割を終える。
次回実行時は `learnings.md` のみを検索するため、リネーム済みファイルは自動的にスキップされる。
