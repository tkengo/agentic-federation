...
2. ワークフロー定義を読む:
   ...
   ```
3. `fed state read status` の値に応じて、ワークフロー定義の該当ステートのアクションを実行する。
...
エージェントは起動時に自分のプロンプトを読み込み済み。
アーティファクトの読み書き方法、完了報告の送信先は各エージェントが自分で知っている。

タスクメッセージには以下だけを含める：
1. タスクの `message` フィールドの内容（コア指示）
2. 必要に応じてコンテキスト情報（例: レビューフィードバックがあること）
