...
6. `fed notify agents.5 "'fed prompt read test-brushup-code-reviewer-gemini' の出力を読んで、コードをレビューしてください。"` を実行してGeminiにレビューを依頼する
7. `fed notify agents.6 "'fed prompt read test-brushup-code-reviewer-codex' の出力を読んで、コードをレビューしてください。"` を実行してCodexにレビューを依頼する
...
8. `fed notify agents.5 "コードが修正されています。再レビューしてください。"` を実行してGeminiに再レビューを依頼する
9. `fed notify agents.6 "コードが修正されています。再レビューしてください。"` を実行してCodexに再レビューを依頼する
...
コードレビューフェーズではCodex(テスト意図の保全、アサーション強度、テスト実行結果)とGemini(重複排除の品質、可読性、フレームワーク活用、一貫性)がそれぞれの観点からレビューをします。レビュー結果を受け取った場合は、以下の手順に従って対応してください。
