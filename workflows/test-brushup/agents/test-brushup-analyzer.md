...
6. **質問をする時は先に通知を送ってAskUserQuestionツールで質問する。** 人間に質問を投げる際には必ず `fed waiting-human set --reason "質問があります" --notify` を実行して人間に通知を送ってから、AskUserQuestionツール経由で質問すること。
7. **なぜその質問をするのか必ず理由を説明する。** ユーザーが質問の意図を理解できるようにする。
8. **質問時は必ず通知を送る。** ユーザーへの質問を送信したら、即座に `fed waiting-human set` で待機状態にする（後述）。
9. **人間のフィードバックに基づく変更は「人間による確定事項」として記録する。**（後述）
