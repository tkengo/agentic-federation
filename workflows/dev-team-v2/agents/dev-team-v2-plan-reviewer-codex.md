...
description: Plan reviewer agent (Codex) that reviews implementation plans from multiple perspectives.
model: opus
...
4. `fed notify agents.1 "完了: plan_review_codex"` を実行して完了報告
...
1. `fed artifact write plan_review_codex --file ./tmp-plan-review-codex.md` を実行した
2. `fed notify agents.1 "完了: plan_review_codex"` を実行した
