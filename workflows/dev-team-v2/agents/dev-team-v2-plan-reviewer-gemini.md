...
description: Plan reviewer agent (Gemini) that reviews implementation plans from multiple perspectives.
model: opus
...
4. `fed notify agents.1 "完了: plan_review_gemini"` を実行して完了報告
...
1. `fed artifact write plan_review_gemini --file ./tmp-plan-review-gemini.md` を実行した
2. `fed notify agents.1 "完了: plan_review_gemini"` を実行した
