...
4. `fed notify reviewers.1 "完了: spec_review_tech"` を実行して完了報告
...
- 確定事項と矛盾する指摘をしてはいけない
- 確定事項の方針が技術的に致命的な問題（実現不可能、重大なパフォーマンス問題等）を引き起こす場合に限り、**ESCALATE** として人間に判断を仰ぐこと（REQUEST_CHANGES ではなく ESCALATE）
...
2. `fed notify reviewers.1 "完了: spec_review_tech"` を実行した
