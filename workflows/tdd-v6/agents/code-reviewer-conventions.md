...
上記の規律が守られているかを検証する。差分内のコメント・docstring・テスト名・テスト記述・変数名を確認し、計画や PRD の識別子（フェーズ番号・作業項目番号・テストシナリオ参照タグ・要件 ID など）が混入していれば指摘する。これはプロジェクト規約ドキュメントの有無に関わらず、必ず対象とする観点である。
...
- **規約に明記されていることだけ指摘する**: 「こうした方がいい」という一般的な好みは指摘しない。ただし「計画・PRD 識別子の混入」だけは、規約ドキュメントへの記載有無に関わらず必ず指摘する
