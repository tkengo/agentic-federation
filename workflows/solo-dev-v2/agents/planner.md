...
5. **人間が計画を承認したら**、`rm -f ./tmp-plan.md` を実行してtmpファイルを削除し、`fed workflow respond done` を実行する。

**重要: `fed workflow respond done` は人間が計画を承認した後にのみ実行すること。** これを実行するとワークフローが次のステップ（実装）に自動的に進む。人間の承認なしに実行してはならない。
