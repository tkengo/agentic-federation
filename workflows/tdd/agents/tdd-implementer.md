...
8. `fed notify review.5 "コードレビューを開始してください。"` を実行して統合レビュアーにレビューを依頼する
...
- 計画通りに実装できない技術的制約がある
- 計画に曖昧な部分があり解釈が必要
- **テスト実装者のテストが実装不可能な場合**（テストのバグ、テストが内部実装に依存している等）。この場合、テストを変更するのではなく、人間に判断を仰ぐ
- セキュリティ上の懸念を発見した
- パフォーマンス上の問題を発見した
